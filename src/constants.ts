export const HONEY_FORM_ERRORS = {
  emptyFormFieldsRef: '[honey-form]: The `formFieldsRef` value is null',
  parentFieldValue:
    '[honey-form]: The `value` of `parentField` must be of type Array when using it.',
  parentFieldFormIndex:
    '[honey-form]: When using `parentField` with an existing default value, the `formIndex` option must be provided. Please specify the `formIndex` when rendering the child form.',
  submitHandlerOrOnSubmit:
    '[honey-form]: To submit the form, either provide a `submitHandler` function or implement an `onSubmit` callback function',
};
